...
3. 混合型方法(Hybird):  
   结合了上述两种方法：learning-based和optimization-based的方法，如 (d) 所示。
...
[55] 什么是 FID
```
   FID是衡量生成图像质量的指标，FID是从原始图像的计算机视觉特征的统计方面，来衡量两组图像的相似度，是计算真实图像和生成图像的特征向量之间距离的一种度量。
   这种视觉特征是使用 Inception v3 图像分类模型提取特征并计算得到的。FID 在最佳情况下的得分为 0.0，表示两组图像相同。分数越低代表两组图像越相似，或者说二者的统计量越相似。
   计算：使用Inception v3 图像分类模型将每个图像预测为2048个激活特征（向量），计算图像特征向量的均值mu和协方差矩阵C，FID=|| mu_1-mu_2 ||^2+Tr(C_1+C_2-2*sqrt(C_1*C_2))，|| mu_1-mu_2 ||^2代表两个平均向量差的平方和。
```
...
```
   在BP算法基于梯度下降策略中，多个激活函数偏导数的连乘，和多个权重参数的连乘。如果激活函数求导后与权重相乘的积大于1，那么随着层数增多，求出的梯度更新信息将以指数形式增加，即发生梯度爆炸；如果此部分小于1，那么随着层数增多，求出的梯度更新信息将会以指数形式衰减，即发生梯度消失。
...
解决方法：
   预训练加微调：先寻找局部最优，然后整合起来寻找全局最优；
   梯度剪切、正则：设置一个梯度剪切阈值，更新梯度时，如果梯度超过这个阈值，那么就将其强制限制在这个范围之内，防止梯度爆炸，或者权重正则化，常用的是L1正则和L2正则；
   合理的激活函数、权值初始化：如果激活函数的导数为1，那么就消除了激活函数偏导数的影响，只需考虑权值即可。
   Batch Normalization：与不使用BN相比，每层的反向传播过程，增加了一个基于标准差的矩阵1/σ。如果权重W较小，使得标准差小，相对1/σ较大，反之，权重W较大，使得标准差大，相对1/σ较小，于是在加入了BN的梯度反向传播过程中就不容易出现梯度消失或爆炸，梯度始终保持在一个合理的范围内。
   残差网络结构：解决梯度消失的问题，ResNet可以有效减少梯度相关性的衰减

```
[13] 学习率过大对训练过程有何影响？什么是学习率调度策略？如何选择？
```
过大的影响：导致参数更新步幅过大，迈过了很多候选参数，有可能会越过最优值。会导致参数优化过程中损失函数值震荡（或在最终的极优值两侧来回摆动），导致网络不能收敛。

等间隔调整学习率: 学习率下降间隔数, 学习率调整倍数
指数衰减:
自然指数衰减：自然指数衰减是指数衰减的一种特殊情况，学习率也是跟当前的训练轮次指数相关，只不过以 e 为底数
多项式衰减:定义一个初始的学习率，一个最低的学习率，按照设置的衰减规则，学习率从初始学习率逐渐降低到最低的学习率，并且可以定义学习率降低到最低的学习率之后，是一直保持使用这个最低的学习率，还是到达最低的学习率之后再升高学习率到一定值，然后再降低到最低的学习率（反复这个过程）。
余弦衰减: 余弦衰减的衰减机制跟余弦函数相关，形状也大体上是余弦形状
倒数衰减：倒数衰减指的是一个变量的大小与另一个变量的大小成反比的关系，具体到神经网络中就是学习率的大小跟训练次数有一定的反比关系。

```
[29] 简述 FPN 的基本结构
```
自下向上的路径：以2为缩放步长，计算出多个尺度的特征。许多层产生相同大小的特征图，将这些输出相同大小特征图的层称为相同网络阶段，然后选择每个阶段的最后一层的输出作为特征图的参考集。
自上向下的路径和横向连接：上采样倍率设置为2倍，通过最高层次的特征上采样产生更高分辨率的图，然后使用逐元素加法将对应分辨率大小的自下到上的特征与该上采样特征合并。不断迭代该过程最后形成最好的分辨率映射。
...
```
Crop：黑色背景的无效区域，带来了很大的运算量开销。面对这种图片，一般先将区域限制到它的有效区域。
Cutout：在训练时随机把图片的一部分减掉，这样能提高模型的鲁棒性。它的来源是计算机视觉任务中经常遇到的物体遮挡问题。
Random image cropping and patching：方法随机裁剪四个图片的中部分，然后把它们拼接为一个图片，同时混合这四个图片的标签。
Random erasing：用随机数或者数据集中像素的平均值替换原来的像素值。物体遮挡问题。
Mixup training：就是每次取出2张图片，然后将它们线性组合，得到新的图片，以此来作为新的训练样本，进行网络的训练。Mixup方法主要增强了训练样本之间的线性表达，增强网络的泛化能力。
Mirroring : 图片的镜像倒转，物体检测方法。
Rotation：旋转，物体检测方法。
Color shifting：在实践中，R、G和B的值是根据某种概率分布来决定的。这么做的理由是，可能阳光会有一点偏黄，或者是灯光照明有一点偏黄，这些可以轻易的改变图像的颜色，但是对物体的识别，或者是内容的识别，以及标签，还是保持不变的。颜色失真或者是颜色变换方法，这样会使得你的学习算法对照片的颜色更改更具鲁棒性。
添加随机噪声：随机噪声是在原来的图片的基础上，随机叠加一些噪声
缩放变形：随机选取图像的一部分，然后将其缩放到原图像尺度
模糊：减少各像素点值的差异实现图片模糊，实现像素的平滑化。
SimplexNoiseAlpha：产生连续单一噪声的掩模后，将掩模与原图像混合。
FrequencyNoiseAlpha：在频域中用随机指数对噪声映射进行加权，再转换到空间域。
增强对比度: 有一些图像可能比较模糊，对比度增强可能有很好的效果
...
```
优化是指改变x以最小化或最大化某个函数f(x)的任务，记为x*=arg minf(x)
如：梯度下降法、牛顿法

Feedforward：是有确定的输入和输出，但是决定输入和输出的函数不清楚，通过神经网络的训练最终得出输入和输出之间的函数关系。
如：y=ax这样一个函数，x和y确定，但是系数a是多少不清楚。
